...
$(document).ready(function() {
    ...
    function fetchProperties(page, sortproperty,propertyType) {
        $.ajax({
            url: `http://localhost:3000/listproperties?page=${page}&sortproperty=${sortproperty}&type=${propertyType}&pagename=${pagename}`,
            type: 'GET',
            dataType: 'json',
    ...
    fetchProperties(1, sortproperties.val(),selectedPropertyType);

    // Handle pagination link clicks
    paginationContainer.on('click', 'a', function(e) {
        e.preventDefault();
        const page = parseInt($(this).attr('data-page'));
        fetchProperties(page, sortproperties.val(),selectedPropertyType);
    });
...
propertyTypesList.on('click', 'a', function(e) {
  e.preventDefault();
  const clickedPropertyType = $(this).data('property-type');
  if (selectedPropertyType !== clickedPropertyType) {
      selectedPropertyType = clickedPropertyType;
      propertyTypesList.find('a').removeClass('active');
